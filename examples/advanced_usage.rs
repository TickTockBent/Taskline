...
use taskline::{Scheduler, Task, TaskConfig, SchedulerConfig, Result as TasklineResult};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(task) = scheduler.get_task(&manual_task_id).await {
        ...
        for id in &task_ids {
            if let Some(task) = scheduler.get_task(id).await {
                ...
                info!("Task {} stats: {} executions, {} successes, {} failures",
    ...
    if let Some(uptime) = scheduler.uptime().await {
        info!("Scheduler has been running for {} seconds", uptime.num_seconds());
    }
    ...
    info!("Example complete. Final counter value: {}", counter.get().await);
    Ok(())
}
